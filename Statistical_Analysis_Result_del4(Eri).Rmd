...
author: "James Davin, Rui Gong, Jiemin Yang, Eri Grezda"
date: "2024-03-29"
output:
  html_document:
    df_print: paged
...
# VIF for m.mls
vif_mls <- car::vif(m.mls)

# VIF for m.interact
vif_interact <- car::vif(m.interact)

# VIF for m.root_interact
vif_root_interact <- car::vif(m.root_interact)

# Print VIF values
print("VIF for Model m.mls:")
print(vif_mls)

print("VIF for Model m.interact:")
print(vif_interact)

print("VIF for Model m.root_interact:")
print(vif_root_interact)

# Function to create bar plot for VIF values
plot_vif <- function(model_name, vif_values) {
  ggplot(data.frame(Predictor = names(vif_values), VIF = vif_values), aes(x = Predictor, y = VIF)) +
    geom_bar(stat = "identity", fill = "skyblue") +
    labs(title = paste("Variance Inflation Factors (VIF) for", model_name),
         x = "Predictor", y = "VIF") +
    theme_minimal() +
...
plot_m.mls <- plot_vif("m.mls", vif_mls)
plot_m.interact <- plot_vif("m.interact", vif_interact)
plot_m.root_interact <- plot_vif("m.root_interact", vif_root_interact)

plot_m.mls
plot_m.interact
plot_m.root_interact
```
The first model, a multiple linear regression, shows low multicollinearity among its predictors with VIF values of 1.35 for x1, 1.01 for x2, and 1.34 for x3. Because each of these values is below 5, It indicates that the predictors are relatively independent of each other. In our second model which includes interaction terms, our model retains a relatively low mulitcollinearity. There is a relatively high variance inflation factor in our interaction term, 11.35 for x1:x3, however we can assume this is redundant because the interaction term is made up of two predictors that are already in the data set and therefore it is inherently co linear. For the same reason we see the VIF values for x1 increased to 5.89 and x3 increased to 3.80 and x2 remained nearly constant at 1.01 because it was not part of the interaction term. Overall our interaction model shows low levels of multicollinearity, however we are examining the predictor x1 further because its VIF value is greater than 5.
